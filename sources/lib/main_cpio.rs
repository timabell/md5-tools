...
pub fn main() -> (Result<(), io::Error>) {
...

    {
        ...
        _parser.refer(&mut _nice_level).add_option(
            &["--nice"],
            argparse::Parse,
            "OS process scheduling priority (i.e. `nice`) (19 by default)",
        );
    ...
    loop {
        ...
        if (_metadata.mode() & libc::S_IFMT) == libc::S_IFREG {
            if (_metadata.file_size() > 0) || (_metadata.nlink() <= 1) {
                ...
                eprintln!(
                    "[ww] [7c9f8eb7]  hard-link detected: `{}`;  ignoring!",
                    _metadata.name()
                );
            ...
            let _path_prefix = if _path.starts_with("/") {
                ""
