...
pub fn main() -> (Result<(), io::Error>) {
    ...
    let _output_descriptor = if if _output_path == path::Path::new("-") {
        ...
        true
    } else if _output_path.starts_with(path::Path::new("/dev/fd"))
        || _output_path.starts_with(path::Path::new("/proc/self/fd"))
    {
        true
    } else if _output_path.starts_with(path::Path::new("/dev"))
        ...
        || _output_path.starts_with(path::Path::new("/sys"))
    {
        ...
        match _output_path_with_transformer {
            None => None,
            Some((_output_path, None)) => Some(_output_path),
            Some((_output_path_base, Some(_transformer))) => {
                ...
                if _transformer {
                    let _output_path =
                        _output_path_base.join(_output_timestamp + _output_path_suffix);
                    Some(_output_path)
                } else {
    ...
    if let Some((ref _output_path, _)) = _output_path_and_tmp {
        eprintln!(
            "[ii] [8cc8542c]  creating `{}`...",
            _output_path.to_string_lossy()
        );
    }
    let (_output_file, _output_stat) = if let Some((_, ref _output_path_tmp)) = _output_path_and_tmp
    {
        let mut _output_file = fs::OpenOptions::new()
            .create_new(true)
            .write(true)
            .open(_output_path_tmp)?;
        if let Err(_error) = _output_file.set_permissions(fs::Permissions::from_mode(0o600)) {
            eprintln!(
                "[ii] [c892431d]  failed making read-only `{}`:  {};  ignoring!",
                _output_path_tmp.to_string_lossy(),
                _error
            );
        }
    ...
    let _sink = StandardHashesSink::new(
        _output_file,
        _flags.format_flags.zero,
        _flags.format_flags.path,
    );
    ...
    let mut _batch = if _flags.batch_size > 1 {
        Some(
            Vec::<(walkdir::DirEntry, fs::Metadata, DirEntryOrder)>::with_capacity(
                _flags.batch_size,
            ),
        )
    } else {
    ...
    loop {
        ...
        let _entry = match _walker.next() {
            ...
            Some(Err(_error)) => {
                ...
                if _flags.report_errors_to_stderr {
                    message!(
                        _progress,
                        "[ee] [a5e88e25]  failed walking path `{}`: `{}`!",
                        _path.to_string_lossy(),
                        _error.io_error().unwrap_or(&_unknown_error)
                    );
                }
        ...
        let _metadata = match _entry.metadata() {
            ...
            Err(_error) => {
                ...
                if _flags.report_errors_to_stderr {
                    message!(
                        _progress,
                        "[ee] [96d2838a]  failed walking path `{}`: `{}`!",
                        _entry.path().to_string_lossy(),
                        _error.io_error().unwrap_or(&_unknown_error)
                    );
                }
                if _flags.report_errors_to_sink {
        ...
        if _metadata.is_dir() {
                ...
                match fs::symlink_metadata(&_skip_path) {
                    ...
                    Err(_error) => {
                        if _flags.report_errors_to_stderr {
                            message!(
                                _progress,
                                "[ee] [0c67e368]  failed skipping path `{}`: `{}`!",
                                _entry.path().to_string_lossy(),
                                _error
                            );
                        }
            ...
            if let Some(ref _progress) = _progress {
                _progress
                    .folder
                    .set_message(_entry.path().to_string_lossy().into_owned());
                _progress.folder.tick();
            }
        }
    ...
    if let Some((ref _output_path, ref _output_path_tmp)) = _output_path_and_tmp {
        if let Err(_error) = _output_file.set_permissions(fs::Permissions::from_mode(0o400)) {
            eprintln!(
                "[ii] [abfed219]  failed making read-only `{}`:  {};  ignoring!",
                _output_path_tmp.to_string_lossy(),
                _error
            );
        }
...
fn execute_hasher<Sink: HashesSink>(
    _context: HasherContext<Sink>,
    _relative_path: Option<path::PathBuf>,
) -> (Result<(), io::Error>) {
    macro_rules! message {
		( $( $token : tt )+ ) => (
    ...
    loop {
        ...
        let mut _file = match _open.open(_path) {
            ...
            Err(_error) => {
                ...
                if _context.flags.report_errors_to_stderr {
                    message!(
                        "[ee] [42f1352f]  failed opening file `{}`: `{}`!",
                        _path.to_string_lossy(),
                        _error
                    );
                }
                if _context.flags.report_errors_to_sink {
                    _sink.handle(
                        _path_for_sink,
                        _context.flags.hashes_flags.algorithm.invalid_raw,
                    )?;
                    _sink.flush()?;
                }
        ...
        _hash_buffer.clear();
        match digest(
            _context.flags.hashes_flags.algorithm,
            &mut _file,
            &mut _hash_buffer,
        ) {
            Ok(()) => {
            ...
            Err(_error) => {
                ...
                if _context.flags.report_errors_to_stderr {
                    message!(
                        "[ee] [1aeb2750]  failed reading file `{}`: `{}`!",
                        _path.to_string_lossy(),
                        _error
                    );
                }
