...
pub fn main() -> (Result<(), io::Error>) {
    let mut _hashes_flags = HashesFlags { algorithm: &MD5 };
...

    {
        ...
        _parser
            .refer(&mut _path_left)
            .add_argument("dataset-a", argparse::Parse, "source file for dataset A")
            .required();
        _parser
            .refer(&mut _path_right)
            .add_argument("dataset-b", argparse::Parse, "source file for dataset B")
            .required();
    ...
    let mut _tokens = Tokens::new(
        _hashes_flags.algorithm.empty,
        _hashes_flags.algorithm.invalid,
    );
    let _record_pattern = regex::bytes::Regex::new(_hashes_flags.algorithm.pattern).unwrap();
    let _source_left = load(
        _path_left.as_ref(),
        &mut _tokens,
        &_record_pattern,
        _format_flags.zero,
        _compression_flags.algorithm,
    )?;
    let _source_right = load(
        _path_right.as_ref(),
        &mut _tokens,
        &_record_pattern,
        _format_flags.zero,
...
fn report_source_statistics(_tag: char, _source: &Source, _statistics: &SourceStatistics) {
    ...
    if _statistics.duplicate_paths != 0 {
        ...
        println!(
            "##      * distinct paths        : {:8}",
            _statistics.distinct_paths
        );
        println!(
            "##      * unique paths          : {:8}",
            _statistics.unique_paths
        );
        println!(
            "##      * duplicate paths       : {:8}",
            _statistics.unique_paths
    ...
    println!(
        "##      * distinct hashes       : {:8}",
        _statistics.distinct_hashes
    );
    println!(
        "##      * unique hashes         : {:8}",
        _statistics.unique_hashes
    );
    println!(
        "##      * duplicate hashes      : {:8}",
        _statistics.duplicate_hashes
    );
    println!("##    * files");
    println!(
        "##      * unique files          : {:8}",
        _statistics.unique_files
    );
    println!(
        "##      * duplicate files       : {:8}",
        _statistics.duplicate_files
    );
    println!(
        "##      * empty files           : {:8}",
        _statistics.empty_files
    );
    println!(
        "##      * invalid files         : {:8}",
        _statistics.invalid_files
    );
...
fn report_diff_statistics(_tag_left: char, _tag_right: char, _diff: &Diff) {
    ...
    println!(
        "##      * distinct hashes       : {:8}",
        _diff.by_hash_statistics.distinct
    );
    println!(
        "##      * unique hashes in ({})  : {:8}",
    ...
    println!(
        "##      * common hashes         : {:8}",
        _diff.by_hash_statistics.matching + _diff.by_hash_statistics.conflicting
    );
    println!(
        "##        * matching paths      : {:8}",
        _diff.by_hash_statistics.matching
    );
    println!(
        "##        * conflicting paths   : {:8}",
        _diff.by_hash_statistics.conflicting
    );
    println!("##    * paths");
    println!(
        "##      * distinct paths        : {:8}",
        _diff.by_path_statistics.distinct
    );
    println!(
        "##      * unique paths in ({})   : {:8}",
        _tag_left, _diff.by_path_statistics.unique_left
    );
    println!(
        "##      * unique paths in ({})   : {:8}",
        _tag_right, _diff.by_path_statistics.unique_right
    );
    println!(
        "##      * common paths          : {:8}",
        _diff.by_path_statistics.matching + _diff.by_path_statistics.conflicting
    );
    println!(
        "##        * matching hashes     : {:8}",
        _diff.by_path_statistics.matching
    );
    println!(
        "##        * conflicting hashes  : {:8}",
...
fn report_diff_entries(_tag_left: char, _tag_right: char, _diff: &Diff, _tokens: &Tokens) {
    ...
    fn print_pairs(
        _pairs: &mut Vec<(char, char, PathKey, HashKey)>,
        _tokens: &Tokens,
        _sort_by_path: bool,
    ) {
        println!();
        if _sort_by_path {
            _pairs.sort_unstable_by_key(|_x| {
                (
                    _tokens.order_of_path(_x.2),
                    _x.1,
                    _tokens.order_of_hash(_x.3),
                    _x.0,
                )
            });
        } else {
        ...
        for &(_slug, _tag, _path, _hash) in _pairs.iter() {
            println!(
                ...
                _slug,
                _tag,
                _tokens.select_hash(_hash),
                _tokens.select_path(_path).to_string_lossy()
            );
        }
        println!();
    }
...
fn index<'a>(_source: &'a Source, _tokens: &'a Tokens) -> (SourceIndex<'a>, SourceStatistics) {
    ...
    let mut _index_by_hash: HashMap<HashKey, Vec<&SourceRecord>> =
        HashMap::with_capacity(_records.len());
    let mut _index_by_path: HashMap<PathKey, Vec<&SourceRecord>> =
        HashMap::with_capacity(_records.len());

    let mut _records_count = 0;
    for _record in _records.iter() {
        _index_by_hash
            .entry(_record.hash)
            .or_default()
            .push(_record);
...
fn diff(
    _source_left: &Source,
    _index_left: &SourceIndex,
    _source_right: &Source,
    _index_right: &SourceIndex,
    _tokens: &Tokens,
) -> (Diff) {
    let mut _hashes =
        Vec::with_capacity(cmp::max(_index_left.by_hash.len(), _index_right.by_hash.len()) * 3 / 2);
    let mut _paths =
        Vec::with_capacity(cmp::max(_index_left.by_path.len(), _index_right.by_path.len()) * 3 / 2);
    ...
    for &_hash in _hashes.iter() {
        let _records_left = _index_left
            .by_hash
            .get(&_hash)
            .map(|_records| {
                _records
                    .iter()
                    .map(|_record| _record.path)
                    .collect::<Vec<PathKey>>()
            })
            .map(|mut _values| {
                _values.sort_unstable_by_key(|&_x| _tokens.order_of_path(_x));
                _values
            });

        let _records_right = _index_right
            .by_hash
            .get(&_hash)
            .map(|_records| {
                _records
        ...
        let _entry = match (_records_left, _records_right) {
            ...
            (None, None) => unreachable!("[6deb2aea]"),
        };
    ...
    for &_path in _paths.iter() {
        let _records_left = _index_left
            ...
            .map(|_records| {
                _records
                    .iter()
                    .map(|_record| _record.hash)
                    .collect::<Vec<HashKey>>()
            })
            .map(|mut _values| {
                _values.sort_unstable_by_key(|&_x| _tokens.order_of_hash(_x));
                _values
            });
